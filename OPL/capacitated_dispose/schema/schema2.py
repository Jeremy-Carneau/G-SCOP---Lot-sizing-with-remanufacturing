...
def plot_graph():
    ...
    labels.update({f"label{dl}" : "dl", f"label{dr}" : "dr", f"label{pn}" : "pn"})
    ...
    dashed_edges = [(f"label{dl}", f"R{dl}"),(f"label{dr}", f"R{dr}"),(f"label{pn}", f"R{pn}")]
